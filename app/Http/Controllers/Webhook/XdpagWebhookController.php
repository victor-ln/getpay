...
class XdpagWebhookController extends Controller
{
    use ToastTrait;
    ...
    public function handleWebhook(Request $request)
    {
        ...
        // if (!$this->verifySignature($request->getContent(), $request->header("x-signature"))) {
     ...
     */
    private function handlePayinConfirmation(Payment $payment, array $webhookData)
    {
        ...
        if ($transactionVerified['data']['status'] !== 'FINISHED') {
    ...
    private function sendOutgoingWebhook(int $accountId, Payment $payment, $providerResponse): void
    {
        ...
        try {
            ...
            $payloadData = [
                ...
                'endToEndId' => $responseData['endToEnd'] ?? null,
            ...
            if ($payment->status === 'paid') {
                ...
                $payloadData['metadata'] = [
                    'authCode' => $responseData['metadata']['authCode'] ?? null,
                    'amount' => $payment->amount,
                    'paymentDateTime' => $responseData['metadata']['paymentDateTime'] ?? null,
                    'pixKey' => $responseData['metadata']['pixKey'] ?? null,
                    'receiveName' => $responseData['metadata']['receiveName'] ?? null,
                    'receiverName' => $responseData['metadata']['receiverName'] ?? null,
                    'receiverBankName' => $responseData['metadata']['receiverBankName'] ?? null,
                    'receiverDocument' => $responseData['metadata']['receiverDocument'] ?? null,
                    'receiveAgency' => $responseData['metadata']['receiveAgency'] ?? null,
                    'receiveAccount' => $responseData['metadata']['receiveAccount'] ?? null,
                    'payerName' => $responseData['metadata']['payerName'] ?? null,
                    'payerAgency' => $responseData['metadata']['payerAgency'] ?? null,
                    'payerAccount' => $responseData['metadata']['payerAccount'] ?? null,
                    'payerDocument' => $responseData['metadata']['payerDocument'] ?? null,
                    'createdAt' => $payment->created_at->toIso8601String(),
                    'endToEnd' => $responseData['metadata']['endToEnd'] ?? null,
                ];
    ...
    private function verifySignature(string $payload, ?string $receivedSignature): bool
    {
        ...
        $secretKey = config("services.webhook.secret"); // Exemplo: config/services.php -> ["webhook" => ["secret" => env("WEBHOOK_SECRET")]]
        ...
        // Normalizar quebras de linha (como discutimos)
        $normalizedPayload = str_replace("\r\n", "\n", $payload);

        // Calcular assinatura esperada
        $expectedSignature = hash_hmac("sha256", $normalizedPayload, $secretKey);
